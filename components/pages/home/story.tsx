...
type StoryProps = {
  ...
  HomepageCopy,
  'ctaContact' | 'ctaHistory' | 'description' | 'eyebrow' | 'highlightsTitle' | 'lead' | 'title'
>
...
export function Story({
  ...
  return (
    <section className='relative z-[3] bg-[rgba(10,48,41,0.98)] px-[1.2rem] py-[clamp(3.6rem,8.5vw,6.4rem)]'>
      <div className='mx-auto grid w-full max-w-[70rem] gap-[clamp(1.6rem,4vw,4rem)] md:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] md:items-start'>
        <div className='grid content-start'>
          <p className='mb-[0.85rem] font-sans text-[0.72rem] uppercase tracking-[0.16em] text-[rgba(201,218,208,0.64)]'>
            {eyebrow}
          </p>
          <h1 className='max-w-[10ch] font-serif text-[clamp(2.6rem,8vw,5.5rem)] leading-[0.92] tracking-[-0.01em] text-balance'>
            {title}
          </h1>
          ...
          <div className='mt-[0.2rem] grid border-t border-[rgba(178,201,190,0.22)]'>
            {spotlightLinks.map((link, index) => (
              <Link
                key={link.href}
                href={link.href}
                className='grid gap-[0.3rem] border-b border-[rgba(178,201,190,0.18)] py-4 transition-[opacity,transform] duration-200 hover:translate-x-1 md:grid-cols-[auto_minmax(0,0.7fr)_minmax(0,1fr)] md:gap-4 md:items-start'
              >
                <span className='font-sans text-[0.68rem] uppercase tracking-[0.16em] text-[rgba(191,211,201,0.44)]'>

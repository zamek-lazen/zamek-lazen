...
export async function getUpcomingEvents(locale: Locale, limit?: number) {
  ...
  >(UPCOMING_EVENTS_QUERY, {end, locale, offset, today}, {next: {revalidate: 60}})

  return events.map(mapEvent)
}

export async function getNearestEvent(locale: Locale): Promise<LocalizedEvent | null> {
...
export function parseSmsTicketEmbedCode(
  ...
  const iframeMatch = embedCode.match(/<iframe\b[^>]*\bsrc=(["'])(.*?)\1[^>]*>/i)
